...
## Open threads
- ~~Start ramp week 1; build a set/rep log~~ → running: Liftosaur history is the set/rep log (nightly pull). Days 1-3 logged (2026-07-31, 08-10, 08-11); Day 4 is the only session never run. RDL ramp is at session 2 of 3 (`@7`, then `@8`) and now lives in the RPE token, not a comment — see DECISIONS.md.
- ~~**Fixed 4x/week slots: user owes four named day/time slots.**~~ → **NAMED 2026-08-12: Mon = Day 1, Tue = Day 2, Wed = Day 3, Fri = Day 4.** Times still unnamed (days only). Chosen because the rest day lands between the two leg days (Day 3 RDL+BSS, Day 4 leg press+curl), which is where it is worth the most — the two upper days stack instead, and a slightly weaker flat press is a cheaper cost than hamstring strain risk. The rotation order 1→2→3→4 matches the weekday order, so Liftosaur's `nextDay` self-serves from 2026-08-17 with no manual day-picking.
  - Origin: not designed, observed. Week of 2026-08-10 accidentally ran Mon/Tue/Wed/Fri (as Days 2/3/1/4 — out of order, because Day 3's DOMS forced Day 4 off Thursday) and it worked. Naming a pattern the user already executed beats prescribing one he hasn't.
  - Check on 2026-08-17: did the slots survive contact with a normal week? The demonstrated failure mode is adherence via schedule chaos, so **the metric is sessions-per-week, not load progression.** Do not redesign volume before two full weeks on these slots.
